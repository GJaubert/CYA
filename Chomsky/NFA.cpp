...
/*    for (int j = 0; j < estados.size(); j++) {
 ...
 void NFA::Specify(std::string fileout){
   std::ofstream fichero;
   fichero.open(fileout, std::fstream::app);
   fichero << nsimbolos << "\n";
   fichero << "~" << "\n";
   for (int i = 0; i < stoi(nsimbolos); i++) {
     fichero << simbolos[i].simbolo << "\n";
   }   
   fichero << nestados << "\n";
   for (int i = 0; i < stoi(nestados); i++) {
     fichero << estados[i].etiqueta << "\n";
   }
   fichero << inicio << "\n";
   fichero << naceptacion << "\n";
   for (int i = 0; i < stoi(naceptacion); i++) {
     fichero << aceptacion[i].etiqueta << "\n";
   }
   fichero << ntransiciones << "\n";
   for (int i = 0; i < stoi(ntransiciones); i++) {
     fichero << transiciones[i].salida << " " << transiciones[i].label;
     fichero << " " << transiciones[i].destino << "\n";
   }
 }
